...
awk '{print $1"\t"$3/$2}' sigint.txt > fc_$c # repressed mode

echo "Identifying DEG's.." `date | awk '{print $4}'` >> log.txt
awk '{if($2>='"$f"') print $1}' fc_$c > down_deg_$d # specify DEG cut-off for down regulated genes

echo "Calculating node weights.." `date | awk '{print $4}'` >> log.txt
paste <(sort $d | awk '{print $0}') <(awk '{print $2}' fc_$c) | awk '{print $1"\t"$2*$3}' > nw_$d 
...
# Computing repressed path
# For $d
echo "Computing repressed paths.." `date | awk '{print $4}'` >> log.txt
...
cat down_deg_$d | while read i; do echo $i `grep -n ",$i," comma_sorted_shpaths_$d | head -1 | sed 's/:/ /g'` ; done | awk '{if($2>0) print $0}' > tpa_paths_$d
...
* down_deg_$d : Repressed genes based on $f fold change as specified by the user.

* ew_$c & ew_$d : Edge weight of $c & $d used to compute shortest paths.

* fc_$c : Fold change calculated as value of $c/$d.
